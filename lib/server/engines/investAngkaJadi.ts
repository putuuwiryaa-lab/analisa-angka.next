...
import type { InvestFilter, InvestFilterKind } from "./investCatalog";

export const INVEST_ANGKA_JADI_FORMULA_VERSION = "invest-angka-jadi-v1";
...
const INVEST_FILTER_KINDS = new Set<InvestFilterKind>([
  "ai",
  "parity",
  "size",
  "bbfs",
...
export function marketIdOf(market: MarketRow) {
  return String(market.id ?? market.slug ?? market.code ?? market.name ?? "");
}

export function marketNameOf(market: MarketRow) {
...
function isInvestFilterKind(value: string): value is InvestFilterKind {
  return INVEST_FILTER_KINDS.has(value as InvestFilterKind);
}

export function sanitizeInvestPair(value: unknown): TargetPair {
  return value === "depan" || value === "tengah" || value === "belakang" ? value : "belakang";
}

export function sanitizeInvestFilters(value: unknown): InvestFilter[] {
  ...
  return value
    ...
    .filter((item): item is InvestFilter => (
      isInvestFilterKind(item.kind) &&
      Number.isInteger(item.param) &&
      item.param > 0 &&
      item.param <= 9
    ))
...
function runEngine(type: string, data: string[], param: number, targetPair: TargetPair, scope: AnalysisScope = "default") {
  ...
  const safeScope = normalizeScopeForType(type, scope);
...
export async function loadInvestMarketData(marketId: string) {
...
export async function generateInvestAngkaJadi(data: string[], pair: TargetPair, filters: InvestFilter[]) {
...
export async function generateInvestAngkaJadiForMarket(marketId: string, pair: TargetPair, filters: InvestFilter[]) {
  const { market, history, latestResult } = await loadInvestMarketData(marketId);
  const result = await generateInvestAngkaJadi(history, pair, filters);

  return {
    formula_version: INVEST_ANGKA_JADI_FORMULA_VERSION,
