...
export default function SharePrediksiPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchJson<ShareOption[]>("/api/share-predictions/options"), fetchJson<MarketOption[]>("/api/markets")])
      .then(([items, markets]) => {
        if (!active) return;
  ...
  async function handleGenerateRekap() {
    if (!selectedOption || !isRekapBadge(selectedOption)) return;
  ...
  async function handleGenerateBbfs7() {
    if (!selectedOption || !isBbfs7Trial(selectedOption)) return;
    const marketIds = selectedMarketIds();
    ...
    try {
      ...
      const data = await fetchJson<ShareResponse>(`/api/share-predictions/bbfs7-trial?${params.toString()}`);
      setRows(data.rows || []);
    } catch (err) {
  ...
  return (
    <div className="animate-rise pb-8">
      ...
      <section className="depth-1 rounded-3xl border p-4">
        ...
        <pre className="depth-2 max-h-[48svh] min-h-[150px] overflow-y-auto whitespace-pre-wrap break-words rounded-3xl border p-4 font-mono text-[12px] font-bold leading-6 text-text">
