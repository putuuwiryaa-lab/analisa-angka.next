...
export default function SharePrediksiPage() {
  ...
  return (
    <div className="animate-rise pb-8">
      ...
      <section className="depth-1 mb-4 rounded-3xl border p-4"><div className="mb-3 flex items-center justify-between gap-3 px-1"><span className="display text-xs text-text">Pilih Pasaran</span><span className="text-[10px] font-black uppercase tracking-wide text-text-soft">{selectedRows.length}/{rows.length}</span></div>{loadingRows ? <div className="depth-2 rounded-2xl border p-4 text-center text-[11px] font-black uppercase tracking-wide text-text-muted">Memuat pasaran…</div> : rows.length === 0 ? <div className="depth-2 rounded-2xl border p-4 text-center text-[11px] font-black uppercase tracking-wide text-text-muted">Belum ada pasaran</div> : <><div className="mb-3 grid grid-cols-2 gap-2"><button type="button" onClick={selectAllMarkets} className="pressable depth-3 rounded-2xl border px-3 py-2 text-[11px] font-black uppercase tracking-wide text-text-muted hover:border-border hover:bg-white/[0.06]">Pilih Semua</button><button type="button" onClick={clearMarkets} className="pressable depth-3 rounded-2xl border px-3 py-2 text-[11px] font-black uppercase tracking-wide text-text-muted hover:border-border hover:bg-white/[0.06]">Kosongkan</button></div><div className="grid max-h-[420px] grid-cols-2 gap-3 overflow-y-auto pr-1 sm:grid-cols-3">{rows.map((row) => { const key = marketKey(row); const active = selectedMarketKeys.has(key); const result = selectedOption ? rowResultText(selectedOption, row) : "-"; return <button key={key || marketLabel(row)} type="button" onClick={() => toggleMarket(row)} className={active ? "pressable animate-soft-pop accent-bg-soft accent-border group relative flex h-[112px] flex-col overflow-hidden rounded-3xl border text-center" : "pressable animate-soft-pop depth-1 group relative flex h-[112px] flex-col overflow-hidden rounded-3xl border text-center hover:border-border hover:bg-surface-2"}>{active && <span className="accent-bg accent-text-inverse absolute right-2 top-2 z-10 flex h-6 w-6 items-center justify-center rounded-full"><Check size={14} strokeWidth={3.2} /></span>}<div className="depth-2 flex min-h-[48px] items-center justify-center border-b border-border-soft px-2 transition-colors group-hover:bg-white/[0.04]"><span className="display line-clamp-2 text-[10px] leading-4 text-text">{marketLabel(row)}</span></div><div className="flex flex-1 items-center justify-center px-2"><span className="num break-all text-base font-black tracking-[0.04em] text-accent transition-transform duration-150 group-hover:scale-[1.03]">{result}</span></div></button>; })}</div></>}</section>
