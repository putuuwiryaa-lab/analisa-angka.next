...
const PAIR_LABEL: Record<TargetPair, string> = {
  depan: "2D Depan",
  tengah: "2D Tengah",
...
const POSITION_KEY: Record<"AS" | "KOP" | "KEPALA" | "EKOR", "as" | "kop" | "kepala" | "ekor"> = {
  AS: "as",
  KOP: "kop",
  KEPALA: "kepala",
  EKOR: "ekor",
};
const POSITION_PROP: Record<"AS" | "KOP" | "KEPALA" | "EKOR", "offAs" | "offKop" | "offKepala" | "offEkor"> = {
  AS: "offAs",
  KOP: "offKop",
  KEPALA: "offKepala",
  EKOR: "offEkor",
...
function isSuccess(row: EvalRow) {
...
function rowsFor(
  rows: EvalRow[],
  { mode, pair = "belakang", scope = "default", position = "all" }: { mode: string; pair?: TargetPair; scope?: string; position?: string },
) {
  return rows.filter((row) =>
    row.mode === mode &&
    String(row.target_pair || "belakang") === pair &&
    String(row.analysis_scope || "default") === scope &&
    String(row.position || "all").toLowerCase() === position,
  );
}
...
async function loadEvaluations(supabase: ReturnType<typeof createAdminClient>, marketId: string) {
  const { data, error } = await supabase
    ...
    .select("mode,param,position,target_pair,analysis_scope,is_hit,status,evaluated_at")
    .eq("market_id", marketId)
    .in("mode", ["ai", "ai_parity", "ai_size", "bbfs", "mati", "jumlah", "shio"])
...
function buildPairSection(pair: TargetPair, data: string[], evalRows: EvalRow[]) {
  ...
  if (aiPick) {
    state.aiByPair[pair] = resultList(runAi(data, pair, aiPick.param));
    activeFilters += 1;
  }
  ...
  if (bbfsPick) {
    state.bbfsByPair[pair] = resultList(runBbfs(data, pair, bbfsPick.param));
    activeFilters += 1;
  }

  const bbfsGgbkPick = chooseParam(rowsFor(evalRows, { mode: "bbfs", pair, scope: `2d_${pair}` }), "bbfs", [10], "low");
  if (bbfsGgbkPick) {
    state.bbfsByPair[pair] = resultList(runBbfs(data, pair, 10));
    activeFilters += 1;
  }
  ...
  if (jumlahPick) {
    state.jumlahByPair[pair] = resultList(runAnalysis("jumlah", data, jumlahPick.param, { targetPair: pair }));
    activeFilters += 1;
  }

  const shioPick = chooseParam(rowsFor(evalRows, { mode: "shio", pair }), "shio", [1, 2, 3], "high");
  if (shioPick) {
    state.shioByPair[pair] = resultList(runAnalysis("shio", data, shioPick.param, { targetPair: pair }));
    activeFilters += 1;
  }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const requestedIds = String(search.get("marketIds") || "")
      .split(",")
      .map((item) => item.trim())
      .filter(Boolean);
